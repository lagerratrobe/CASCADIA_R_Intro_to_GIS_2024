...
#cities_sf <- st_as_sf(cities, <blank> = c("lon", "lat"), crs = 4326)

# Calculate the distance in KM
#st_<blank>(cities_sf[1,], cities_sf[2,]) |> units::set_units("km")
```
...
#cities_albers <- st_<blank>(cities_sf, crs = 'ESRI:102013')

# Create a 2500km buffer around Paris
#paris_buffer <- st_<blank>(cities_albers[1,], (2500 * 1000))

# Plot
#plot(paris_buffer$<blank>)
#plot(cities_albers$geometry, pch=20, add = TRUE)
#text(st_coordinates(cities_albers), pos=1, cities_albers$<blank>, cex=0.75)
```
...
#st_<blank>(paris_buffer$geometry, cities_albers, sparse = FALSE)
```
